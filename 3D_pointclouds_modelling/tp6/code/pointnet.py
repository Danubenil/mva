...
def my_transform():
    return transforms.Compose([RandomRotation_z(),RandomNoise(), RandomScaling(), ToTensor()])
def test_transforms():
...
if __name__ == '__main__':
    ...
    #model = MLP(classes= len(inv_classes))
    #model = PointNetBasic(classes = len(inv_classes))
    model = PointNetFull(classes = len(inv_classes))
    
    model_parameters = filter(lambda p: p.requires_grad, model.parameters())
    print("Number of parameters in the Neural Networks: ", sum([np.prod(p.size()) for p in model_parameters]))
    model.to(device)
    
    losses_train, losses_test = train(model, device, train_loader, test_loader, epochs = 50, pointnetloss= True, lr = 0.0005, step_size = 10)
